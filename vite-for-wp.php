...
function generate_development_asset_src( object $manifest, string $entry ): string {
	return trailingslashit( $manifest->dev_server ) . $entry;
}
...
function register_vite_client_script( object $manifest ): void {
	...
	$src = trailingslashit( $manifest->dev_server ) . '@vite/client';
...
console.log('[Vite] React Refresh Preamble injected.');
...
function load_development_asset( object $manifest, string $entry, array $options ): ?array {
	$manifest->dev_server = $options['dev_server'];
...
function prepare_asset_url( string $dir ) {
	...
	$manifest_dir        = preg_replace( '/\.vite$/', '', $manifest_dir );
...
function register_asset( string $manifest_dir, string $entry, array $options ): ?array {
	...
	if ( $options['is_dev'] && isset( $options['dev_server'] ) ) {
		$manifest->dev_server = $options['dev_server'];
		$assets               = load_development_asset( $manifest, $entry, $options );
	} else {
		$assets = load_production_asset( $manifest, $entry, $options );
	}
